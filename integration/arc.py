import jax 
import jax.numpy as jnp
import constant as c
import linear as l
import quadratic as q

@jax.jit
def ellipse_arc(ld_params, a, b, z, phi1, phi2):

    uc = 1 - ld_params[0] - 2 * ld_params[1]
    ul = ld_params[0] + 2 * ld_params[1]
    uq = ld_params[1]

    terms = ellipse_terms(a, b, z, phi1, phi2)
    return terms[0] * uc + terms[1] * ul + terms[2] * uq

@jax.jit
def circle_arc(ld_params, r, z, phi1, phi2):

    uc = 1 - ld_params[0] - 2 * ld_params[1]
    ul = ld_params[0] + 2 * ld_params[1]
    uq = ld_params[1]

    terms = circle_terms(a, b, z, phi1, phi2)
    return terms[0] * uc + terms[1] * ul + terms[2] * uq

def ellipse_terms(a, b, z, phi1, phi2):

    return jax.lax.cond(
        jnp.abs(phi2 - phi1 - 2 * jnp.pi) < 1e-6,
        lambda: (2 * c.G_comp(a, b, z), 2 * l.G_comp(a, b, z), 2 * q.G_comp(a, b, z)),
        lambda: ellipse(a, b, z, phi1, phi2)
    )

def circle_terms(r, z, phi1, phi2):

    return jax.lax.cond(
        jnp.abs(phi2 - phi1 - 2 * jnp.pi) < 1e-6,
        lambda: 2 * c.Gc_comp(r, z), 2 * l.Gc_comp(r, z), 2 * q.Gc_comp(r, z),
        lambda: circle(r, z, phi1, phi2)
    )

# this function assumes that both limits are in (0, 2*pi) 
def ellipse(a, b, z, phi1, phi2):

    s1 = jnp.sin(phi1)
    s2 = jnp.sin(phi2)
    c1 = jnp.cos(phi1)
    c2 = jnp.cos(phi2)

    conditions = jnp.array([
        (c1 > 0) & (c2 > 0) & (s1 > s2), # C
        (c1 > 0) & (c2 > 0),             # A/F
        (c1 > 0) & (c2 < 0),             # B
        (c1 < 0) & (c2 < 0),             # D
        (c1 < 0) & (c2 > 0)              # E
    ])

    outcomes = [
        lambda: (
            c.G(a, b, z, phi2, phi1),
            2 * l.G_comp(a, b, z) - l.G(a, b, z, s2, s1),
            q.G(a, b, z, phi2, phi1)
        ),
        lambda: (
            c.G(a, b, z, phi2, phi1),
            l.G(a, b, z, s1, s2),
            q.G(a, b, z, phi2, phi1)
        ),
        lambda: (
            c.G(a, b, z, phi2, phi1),
            l.G_x1(a, b, z, s1) + l.G_x1(a, b, z, s2),
            q.G(a, b, z, phi2, phi1),
        ),
        lambda: (
            c.G(a, b, z, phi2, phi1),
            l.G(a, b, z, s2, s1),
            q.G(a, b, z, phi2, phi1),
        ),
        lambda: (
            c.G(a, b, z, phi2, phi1),
            l.G_ym1(a, b, z, s1) + l.G_ym1(a, b, z, s2),
            q.G(a, b, z, phi2, phi1)
        )
    ]

    ind = jnp.argwhere(conditions, size=1).squeeze()
    return jax.lax.switch(ind, outcomes)

# this function assumes that both limits are in (0, 2*pi) 
def circle(r, z, phi1, phi2):

    s1 = jnp.sin(phi1)
    s2 = jnp.sin(phi2)
    c1 = jnp.cos(phi1)
    c2 = jnp.cos(phi2)

    conditions = jnp.array([
        (c1 > 0) & (c2 > 0) & (s1 > s2), # C
        (c1 > 0) & (c2 > 0),             # A/F
        (c1 > 0) & (c2 < 0),             # B
        (c1 < 0) & (c2 < 0),             # D
        (c1 < 0) & (c2 > 0)              # E
    ])

    outcomes = [
        lambda: (
            c.Gc(a, b, z, phi2, phi1),
            2 * l.Gc_comp(a, b, z) - l.Gc(a, b, z, s2, s1),
            q.Gc(a, b, z, phi2, phi1)
        ),
        lambda: (
            c.Gc(a, b, z, phi2, phi1),
            l.Gc(a, b, z, s1, s2),
            q.Gc(a, b, z, phi2, phi1)
        ),
        lambda: (
            c.Gc(a, b, z, phi2, phi1),
            l.Gc_x1(a, b, z, s1) + l.Gc_x1(a, b, z, s2),
            q.Gc(a, b, z, phi2, phi1),
        ),
        lambda: (
            c.Gc(a, b, z, phi2, phi1),
            l.Gc(a, b, z, s2, s1),
            q.Gc(a, b, z, phi2, phi1),
        ),
        lambda: (
            c.Gc(a, b, z, phi2, phi1),
            l.Gc_ym1(a, b, z, s1) + l.Gc_ym1(a, b, z, s2),
            q.Gc(a, b, z, phi2, phi1)
        )
    ]

    #outcomes = [
    #    lambda: 2 * linear.Gc_comp(r, z) - linear.Gc(r, z, s2, s1),
    #    lambda: linear.Gc(r, z, s1, s2),
    #    lambda: linear.Gc_x1(r, z, s1) + linear.Gc_x1(r, z, s2),
    #    lambda: linear.Gc(r, z, s2, s1),
    #    lambda: linear.Gcc_ym1(r, z, s1) + linear.Gc_ym1(r, z, s2)
    #]

    ind = jnp.argwhere(conditions, size=1).squeeze()
    return jax.lax.switch(ind, outcomes)